...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">校友风采管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>校友风采列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">校友风采列表</h3>							
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('fengguangshiji')">风光事迹<i id="fengguangshijiIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('zaixiaoshijian')">在校时间<i id="zaixiaoshijianIcon" class="fa fa-sort"></i></th>
											<th>操作</th>
										</tr>
									</thead>
									<tbody>
	...
	<script>
		...
		function setDataRow(item,number){
			...
			var fengguangshijiCell  = document.createElement('td');

						  //如果图文详情包含图片
			  if(item.fengguangshiji && item.fengguangshiji.indexOf('img') != -1){
				//暂时只考虑图片+文字 和 文字+图片的情况
				var beginIndex = item.fengguangshiji.indexOf('<img');
				var endIndex = item.fengguangshiji.indexOf('>');				
				fengguangshijiCell.innerHTML = item.fengguangshiji.substring(beginIndex, endIndex+1).replace("img","img width='100' height='100'");				
			  }else{
			  	 if(item.fengguangshiji != null && item.fengguangshiji != "" && item.fengguangshiji.length >= 11){
			  		fengguangshijiCell.innerHTML = item.fengguangshiji.substring(0, 10) + "..."; 
			  	}else{
			  		fengguangshijiCell.innerHTML = item.fengguangshiji;
			 	}
			  }
				  


			row.appendChild(fengguangshijiCell);
			var zaixiaoshijianCell  = document.createElement('td');

						 zaixiaoshijianCell.innerHTML = item.zaixiaoshijian;


			row.appendChild(zaixiaoshijianCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("xiaoyoufengcai/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"xiaoyoufengcai");
